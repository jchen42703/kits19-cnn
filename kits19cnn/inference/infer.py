...
class Predictor(Preprocessor):
    ...
    """
    def __init__(self, model, weights_path, out_dir, in_dir=None, coords_csv_path=None, from_logits=True,
        ...
        Attributes:
            ...
            in_dir (str): directory with the input data. Should be the kits19/data directory. Defaults to None.
        ...
        try:
            ...
            if self.in_dir is None:
                raise Exception("in_dir cannot be `None` if coords_csv_path is also None.")
    ...
    def predict(self, evaluate=True):
        ...
        for (i, case) in enumerate(self.cases):
            ...
            if self.coords_csv is None:
                ...
                image = np.load(join(self.in_dir, case, "imaging.npy"))
                label = np.load(join(self.in_dir, case, "segmentation.npy"))
                coords = self.parse_coords_csv(case)
    ...
    def parse_coords_csv(self, case):
        """
        Args:
        ...
        """
        coords_arr = df.loc[case].values # arr: [x_lb, x_ub, y_lb, y_ub, z_lb, z_ub]
        # groups x, y, z into their own sub-lists and appends them to a big list
        coords_list = [[coords_arr[idx], coords_arr[idx+1]] for idx in range(0, 5, 2)]
        # moving z coords to the front
        coords_list.insert(0, coords_list.pop())
        return coords_list
